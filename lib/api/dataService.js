...
export async function fetchGradesByGroup(groupName) {
    try {
        ...
        if (gradesError) {
            console.error('Ошибка при загрузке оценок:', gradesError.message)
            return []
        }
...
export async function createGrade(grade) {
    try {
        console.log('createGrade получил:', grade)
        const { data, error } = await supabase
            .from('grades')
            .insert([{
                ...grade,
        ...
        if (error) {
            console.error('Ошибка при добавлении оценки в БД:', error)
            throw error
        }
...
export async function deleteGrade(gradeId) {
    try {
        ...
        console.error('Ошибка при удалении оценки:', error.message)
        return { success: false, error: error.message }
    }
}
